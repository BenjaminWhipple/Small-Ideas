...
def simulwhisper_args(parser):
    ...
    group.add_argument("--device", default="cuda",
                       help="Torch device for all neural inference (default: cuda).")
    group.add_argument("--whisper-dtype", choices=("float16", "float32"), default="float16",
                       help="Whisper parameter and activation dtype (default: float16).")
...
class SimulWhisperASR(ASRBase):
    ...
    def __init__(self, language, model_path, cif_ckpt_path, frame_threshold, audio_max_len, audio_min_len, segment_length, beams, task, 
        ...
        logger.info(
            "Whisper ready: device=%s dtype=%s",
            self.model.model.device, self.model.model.dtype,
        )
...
class SimulWhisperOnline(OnlineProcessorInterface):
    ...
    def process_iter(self):
        ...
        return {
            ...
            'tokens': tokens,
            'words': ts_words,
            # Expose auto-detection to orchestration clients. Older clients
