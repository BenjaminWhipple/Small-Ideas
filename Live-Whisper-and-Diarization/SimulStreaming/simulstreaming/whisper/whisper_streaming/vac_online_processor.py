...
class VACOnlineASRProcessor(OnlineProcessorInterface):
    ...
    def __init__(self, online_chunk_size, online, device="cuda", min_buffered_length=1):
        ...
        if self.device.type == "cuda" and not torch.cuda.is_available():
            raise RuntimeError("CUDA was requested for Silero VAC but is not available")
        ...
        logger.info(
            "Silero VAC ready: device=%s dtype=%s",
            self.device, parameter.dtype if parameter is not None else torch.float32,
        )
        self.vac = FixedVADIterator(model, device=self.device)
    ...
    def warmup(self):
        self.vac(np.zeros(512, dtype=np.float32))
        self.vac.reset_states()
        if hasattr(self.online, "warmup"):
            self.online.warmup()
    ...
    def process_iter(self):
        if self.is_currently_final:
            ...
            self.current_online_chunk_buffer_size = 0
            return self._mark_final(self.online.process_iter(), False)
        else:
    ...
    def finish(self):
        ...
        if isinstance(ret, list):
            events = self._mark_final(ret, False)
            events.append({"is_final": True})
            return events
        return self._mark_final(ret, True)

    @staticmethod
    def _mark_final(result, is_final):
        events = result if isinstance(result, list) else ([result] if result else [])
        for event in events:
            event["is_final"] = is_final
        if isinstance(result, list):
            return events
        return events[0] if events else ({"is_final": True} if is_final else {})
